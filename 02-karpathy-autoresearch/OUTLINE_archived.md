...
program.md is 114 lines. That's less than a fifth the length of `train.py` which is worth thinking about because the 114 lines do most of the behavioral work for the agent in the system.

Every directive in the file corrects a default behavior that would otherwise derail an overnight run. The reason there are six load-bearing directives (and not sixty) is that Karpathy seems to have written down exactly what he observed breaking and nothing else so instead of instructing the agents to go in a specific direction, the answer was more negative in nature. To understand where the agents were making mistakes and then to tell them not to go down that road.

The boundaries is the first thing the file talks about: the agent touches `train.py` and only `train.py`. Not `prepare.py`, not `results.tsv`. An agent optimizing val_bpb can move the score two ways: make the model better, or weaken the evaluation and so if you push the agent enough, it might "simplify" something in `prepare.py` that it reads as redundant, without understanding that the redundancy is load-bearing. The boundary prevents these kinds of mistakes before they can happen.

Line 99 is the token firewall. Step 4 of the loop:

```
uv run train.py > run.log 2>&1
```

`> run.log 2>&1` redirects all training output to a file. A 300-second training run on an H100 emits roughly 50,000 tokens of output: step losses, throughput figures, VRAM stats, line after line for the full five minutes. If that streams directly into the context window, each experiment costs several thousand tokens just to read what happened. Redirect it to a file and grep for the two values that matter (step 5: `grep "^val_bpb:\|^peak_vram_mb:" run.log`) and the per-experiment cost drops to a dozen lines. Across 700 experiments, that gap is millions of tokens. The redirect rule is in `program.md` rather than hardcoded into `train.py` because the human might eventually want to change what gets grepped.

Lines 70-72 define the TSV schema: commit hash, val_bpb, peak VRAM in megabytes, pass/fail status, and a one-line description. The format is rigid on purpose. The agent reads the full `results.tsv` at step 1 of every iteration, loading its history fresh into context before each new experiment. A schema that varies between entries would force token-expensive parsing before the agent can think about what to try next. Fixed format means the history is always machine-readable, by the machine.
...
This is the most interesting directive in the file, and the hardest to evaluate from the outside. An LLM with no simplicity criterion will accept any change that lowers val_bpb, full stop. It might add 40 lines of per-layer scaling logic to recover 0.001. That change probably doesn't generalize, and it makes every subsequent experiment harder because now the agent must reason through 40 extra lines it didn't design and can't fully interpret. Over hundreds of experiments, complexity stacks up fast. The simplicity criterion filters against that accumulation, meaning the agent sometimes declines a technically passing commit. The git history stays cleaner. The surviving changes tend to be the kind that transfer to larger models.

The part worth sitting with: this is a value judgment, not a rule you can encode. You can't write a test for "ugly." Writing it in plain English is the ONLY way it enters the agent's reasoning at all.
...
An RLHF-trained model's default behavior on an open-ended task with no clear endpoint is to surface a status update and wait for confirmation. That instinct makes models safer in interactive sessions. It makes them useless for overnight runs. You go to sleep. The agent finishes one experiment, decides the situation has developed well, and stops to check in. You wake up to a polite question instead of 80 more data points. The directive has to be that blunt because it's overriding a trained reflex, not just a preference.

What ties all six directives together is Karpathy's framing in the No Priors interview: the human's job is to remove themselves as the bottleneck. Once the loop starts, the only lever left is the next `program.md` edit. That constraint explains why the file is 114 lines and not 1,140 or 11. Every line is there because something specific broke without it. Everything the agent can figure out from context isn't in the file, because putting it in would just burn tokens on things the agent already knows.

The practical implication for anyone adapting this: changing `program.md` is how you change WHAT the agent considers worth doing. Add a second optimization criterion alongside val_bpb and the session starts making different tradeoffs. Remove the simplicity line and come back to a branch dense with baroque attention masks, each of which shaved 0.0003 off the score and added four more things for the next experiment to account for. The agent's architecture is in `train.py`. The agent's values are in `program.md`. Those are different things to edit for different purposes.
